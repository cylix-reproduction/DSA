...
from utils.criterion import Misclassification
...
def load_images():
    ...
    assert original_label != target_label, (
        "original label should not be equal to target label"
    )
    # move to device
    ...
    original_label, target_label = (
        torch.tensor([original_label]).to(device),
...
def attack(cfg: CfgNode):
    ...
    # load attack method
    _attack = get_attack(
        cfg.attack.name, local_models=local_models, constraint=cfg.attack.constraint
    )
    print("\n###### ", _attack, "is Running ######")
